...
class SensorPolicyComparison:
    ...
    def run_simulation_WIQL(self, pivot_df):
            """Simulate the WIQL-UCB policy with UCB exploration"""
            # Remove SN column if present
            if 'SN' in pivot_df.columns:
                pivot_df = pivot_df.drop('SN', axis=1)
            ...
            # Initialize Q-values and visit counts for UCB
            max_state = 30
            Q = np.zeros((N, max_state, 2))  # [sensor, state, action]
            counts = np.zeros((N, max_state, 2))  # Visit counts for UCB
            ...
            for t in range(1, T_steps):
                ...
                # Sensor selection using UCB-based Whittle indices
                ucb_values = np.zeros(N)
                
                for i in range(N):
                    # Current state based on sink estimate rate and delay
                    state_val = self.compute_state(sink_estimates[i][1], delays[i])
                    discrete_state = min(max_state - 1, max(0, self.discretize_state(state_val) - 1))  # 0-indexed
                    
                    # Calculate UCB values for both actions
                    ucb_action_values = np.zeros(2)
                    for a in range(2):
                        if counts[i, discrete_state, a] > 0:
                            exploration = c * np.sqrt(np.log(t + 1) / counts[i, discrete_state, a])
                        else:
                            exploration = c * np.sqrt(np.log(t + 1))
                        ucb_action_values[a] = Q[i, discrete_state, a] + exploration
                    
                    # Whittle index with UCB bonus
                    ucb_values[i] = ucb_action_values[1] - ucb_action_values[0]

                # Select top M sensors by UCB-enhanced Whittle index
                active_sensors = list(np.argsort(ucb_values)[-self.M:])
                ...
                # Update Q-values and counts for UCB
                for i in range(N):
                    ...
                    curr_state = min(max_state - 1, max(0, self.discretize_state(state_val) - 1))  # 0-indexed
                    ...
                    next_state = min(max_state - 1, max(0, self.discretize_state(next_state_val) - 1))

                    # Calculate max Q-value for next state
                    next_val = max(Q[i, next_state, 0], Q[i, next_state, 1])
